...
| `persistMensagem.js` | `applyInboundMediaFields` (puro, type/mídia→campos) + `persistInboundMensagemRow` (I/O: insert + retries de esquema + 23505/merge + fallback; devolve sinal `failed` p/ o `continue` do lote ficar no orquestrador) + `resolveEditedMensagemRow` (isEdit: atualiza texto da linha existente + emite `mensagem_editada`). `tests/webhookPersistMensagem.test.js`, 12 testes | 5 |
| `statusApply.js` | `resolveEffectiveStatus` (puro, ACK sem regressão §4) + `applyAckStatusByWaId` (update por whatsapp_id; recebe `supabaseClient`+ctx; binding fino no controller preserva os 4 call-sites). `tests/webhookStatusApply.test.js`, 11 testes | 5 |
| `fromMeReceivedReconcile.js` | `reconcileFromMeInReceived` — reconcilia eco `fromMe` (crm-* → candidato por mídia/texto, janela 15min, patch, R2 mirror); recebe `supabaseClient`+ctx, devolve `mensagemSalva`/null. `tests/webhookFromMeReceivedReconcile.test.js`, 3 testes (+scanner de imports) | 5 |
| `replyMeta.js` | `buildWebhookReplyMeta` — resolve o `reply_meta` (resposta a X) do payload (muitos formatos de id citado + busca da citada + fallback). `tests/webhookReplyMeta.test.js`, 4 testes | 5 |
...
| **5** | Orquestrador `receberZapi` + shim | **muito alto** | todas as suites da §6 + `node --check` + load da fachada | **EM ANDAMENTO** (2026-09-01). Extraídos (verbatim, todos cobertos por testes): `instanceResolve.js` (tenant), `groupPhoto.js` (callback foto), **`disparoInbound.scheduleInboundDisparoHooks`** (Etapa 8 opt-out+resposta, fire-and-forget), **`historyImport.js`** (import de histórico ao abrir conversa nova), **`crmLeadInbound.js`** (captura de lead CRM) e **`groupSender.js`** (resolve remetente/membro em grupos — contrato→saída, com `tests/webhookGroupSender.test.js`). Controller **3.123 → 2.409** (18 módulos em `webhookInbound/`). Miolo caracterizado em `tests/receberZapiInbound.test.js`. **Ataque ao núcleo (abordagem A) iniciado:** a parte PURA do emit-tail (construção dos payloads `conversa_atualizada` + `nova_mensagem`) foi extraída para `realtimePayload.js` e travada com 11 testes (`webhookRealtimePayload.test.js`) — o emit-tail agora é só I/O + chamada aos builders testados. **Ainda inline (I/O acoplado):** carga da `convRow`, fallback de foto (query clientes), os `io.emit`/`emitirParaUsuariosQuePodemVerConversa`, e o insert/persistência (`persistMensagem` planejado) + reconcile `fromMe`. |
