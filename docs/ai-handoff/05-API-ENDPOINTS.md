...
| `POST /chats/:id/mensagens`, `/pix`, `/encaminhar`, `/contatos`, `/localizacao`, `/ligacao`, `/enquete` | `chatController`; A | JSON específico; persiste mensagem e chama provider (Whapi `sendPoll` em `/enquete`; UltraMSG → 501); pode retornar erro/estado incerto. |
...
| `DELETE /chats/:id/mensagens/:mensagem_id`, `PATCH /chats/:id/mensagens/:mensagem_id`, `POST/DELETE .../reacao` | `excluirMensagem/editarMensagem/enviarReacaoMensagem/removerReacaoMensagem`; A | Apagar chama provider; **editar** (texto ou legenda de mídia) só no Whapi (`editMessage`), janela 15 min, outbound do autor; UltraMSG → 422 `EDIT_NOT_SUPPORTED`. Emite `mensagem_editada`. Campos `mensagens.editada` / `editada_em` (migration `20260907220000`, não aplicar sem autorização). |
...
| `GET/POST /instances`, `PATCH /instances/:id` | Listar/criar/editar instância em `whatsapp_instances`. `GET /instances` em Whapi hidrata `GET /health?wakeup=true` e devolve `connected` + `whapi.partnerEnabled`. |
| `POST /instances/provision-whapi` | SaaS: Partner API cria o canal; grava `instance_id`/`instance_token` no servidor. Sem credencial no body. `company_id` só do JWT. Idempotente se a empresa já tem Whapi. |
...
| `GET /instances/:id/status`, `GET/POST /instances/:id/qrcode` | Health/QR por instância. Whapi: `GET /health?wakeup=true` (`AUTH`/`code 4` = conectado) e `GET /users/login/image` (fallback `GET /users/login`). |
...
`GET /integrations/whatsapp/me` · `GET /integrations/whatsapp/debug-config` · `GET /integrations/whatsapp/debug-status` · `GET /integrations/whatsapp/status` · `GET /integrations/whatsapp/operational-status` · `GET /integrations/whatsapp/qrcode` · `POST /integrations/whatsapp/restart` · `GET /integrations/whatsapp/instances` · `POST /integrations/whatsapp/instances/provision-whapi` · `POST /integrations/whatsapp/instances` · `PATCH /integrations/whatsapp/instances/:id` · `POST /integrations/whatsapp/instances/:id/activate` · `POST /integrations/whatsapp/instances/:id/deactivate` · `POST /integrations/whatsapp/instances/:id/default` · `GET /integrations/whatsapp/instances/:id/status` · `GET /integrations/whatsapp/instances/:id/qrcode` · `POST /integrations/whatsapp/instances/:id/qrcode` · `POST /integrations/whatsapp/instances/:id/restart` · `POST /integrations/whatsapp/instances/:id/phone-code` · `POST /integrations/whatsapp/instances/:id/logout` · `POST /integrations/whatsapp/instances/:id/configure-webhooks` · `GET /integrations/whatsapp/connect/status` · `GET /integrations/whatsapp/connect/qrcode` · `POST /integrations/whatsapp/connect/qrcode` · `POST /integrations/whatsapp/connect/restart` · `POST /integrations/whatsapp/connect/phone-code` · `POST /integrations/whatsapp/configure-webhooks` · `POST /integrations/whatsapp/contacts/sync` · `GET /integrations/whatsapp/messages/sync-old/status` · `POST /integrations/whatsapp/messages/sync-old/cancel` · `POST /integrations/whatsapp/messages/sync-old` · `POST /integrations/whatsapp/groups/sync` · `POST /integrations/whatsapp/sync-all` · `GET /integrations/whatsapp/messages` · `GET /integrations/whatsapp/messages/statistics`
