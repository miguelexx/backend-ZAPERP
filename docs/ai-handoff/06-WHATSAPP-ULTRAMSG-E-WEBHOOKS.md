...
O worker monta `referenceId disp-*`, registra provider id e datas na fila. Webhook/ACK liga o retorno ao item. Mensagem inbound pode ser classificada como resposta à campanha; palavras normalizadas de descadastro geram opt-out e impedem futuros envios. Itens com lease vencido em `enviando` passam a `incerta` na Etapa 9 do working tree; se já têm `provider_message_id` ou `enviado_em`, o worker não reenvia. Decisão manual e reconciliação ficam nas rotas da Etapa 8.
