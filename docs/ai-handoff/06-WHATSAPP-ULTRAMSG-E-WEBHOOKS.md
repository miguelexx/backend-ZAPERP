...
O worker monta `referenceId disp-*`, registra provider id e datas na fila. **Enviada** = o provedor aceitou (ou dry-run). **Entregue/lida** só avançam com ACK (`device`/`read`). O ACK da UltraMSG quase nunca ecoa `referenceId`; o handler de status chama `aplicarStatusDisparoFromWebhook` em todo ACK, casando por `disp-*`, `provider_message_id` ou `mensagem_id` do chat. Mensagem inbound pode ser classificada como resposta à campanha; palavras normalizadas de descadastro geram opt-out e impedem futuros envios. Itens com lease vencido em `enviando` passam a `incerta` na Etapa 9 do working tree; se já têm `provider_message_id` ou `enviado_em`, o worker não reenvia. Decisão manual e reconciliação ficam nas rotas da Etapa 8. Dry-run (`DISPARO_DRY_RUN` default true / live off) nunca chama UltraMSG — a fila fica em enviada e **não chega no WhatsApp**.
