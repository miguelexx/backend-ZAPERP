...
- Disparo: idempotência por `chave_idempotencia` **por execução** (`campanha:{id}:v{versao}:exec:{execucaoId}:dest:{destId}`); UNIQUE global. Claim via `FOR UPDATE SKIP LOCKED`; advisory lock por instância; revisão única por campanha/versão. A Etapa 9 adiciona execução ativa única e recuperação segura de lease, mas não está commitada/aplicação não confirmada.
