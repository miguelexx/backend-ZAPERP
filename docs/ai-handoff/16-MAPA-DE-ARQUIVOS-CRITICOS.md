...
| `controllers/chatController.js` + `controllers/chat/` + `services/chat/` | conversas, atendimento, envio/mídia | crítico | suites `chat*`, mensagem, mídia. Fachada **é shim**; lista/texto/PIX em sub-controllers (podem estar untracked). Mapa: [23](23-CHAT-CONTROLLER-MODULARIZACAO.md). `io` via `req.app.get('io')`. |
...
| `controllers/webhookZapiController.js` | domínio inbound/ACK legado | crítico | ACK/inbound/fromMe; nome legado, caminho ativo. Helpers em `controllers/webhookInbound/`. **Não** mover `receberZapi`/`statusZapi` sem [24](24-WEBHOOK-INBOUND-MODULARIZACAO.md). Não remover por nome. |
