...
9. Conversa **encerrada pelo atendente** permanece `fechada` se o webhook for eco da finalização/última outbound (ou `self` sem `fromMe`). Não reabrir nem reenviar o menu de boas-vindas nesse caso. Demanda nova do cliente continua reabrindo. Código: `closedConversationEcho.js`, `recentClosedConversationGuard.js`, `resolveUltramsgFromMe`.
